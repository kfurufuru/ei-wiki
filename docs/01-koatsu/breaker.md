...
```mermaid
flowchart TD
    subgraph STOP["停電（開放）手順"]
        direction TB
        A1["VCB を開く<br/>（電流を遮断）"] --> A2["VCB 開路を確認<br/>表示灯・機械的インジケータ"]
        A2 -->|"インターロック：VCB 開路確認なしに DS 操作不可"| A3["断路器を開く"]
    end
    subgraph START["投入（充電）手順"]
        direction TB
        B1["断路器を閉じる"] --> B2["断路器の閉路を確認"]
        B2 -->|"インターロック：DS 閉路確認なしに VCB 投入不可"| B3["VCB を閉じる"]
    end
    note["VCB：電流を遮断できる／DS：アーク消弧能力ほぼゼロ<br/>→ 電流は VCB で切り、DS は無電流で操作する"]
```

*VCB と DS の役割差（VCB は電流遮断可・DS は消弧能力なし）が操作順序を決める。開放は VCB→DS、投入は DS→VCB の順で、間に開路・閉路の確認とインターロックが入る。*
