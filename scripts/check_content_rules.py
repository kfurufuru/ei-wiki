...
FORBIDDEN = [
    ...
    ("N16", re.compile(r"事故発生から\s*\**\s*(24|二十四)\s*時間"),
     "事故報告の起算点は「事故の発生を知つた時」（電気関係報告規則第3条第2項）。"
     "「発生から」だと休日夜間発生→翌朝覚知で期限判断を誤る",
     "速報は事故発生から24時間以内"),
    ("N17", _both(r"(サーマル|過負荷保護|整定)", r"定格電流\s*[×x]\s*0\.9"),
    ...
    ("N18", re.compile(r"(記録|報告書).{0,20}(施行規則|法令|法定).{0,20}(により|に基づき)?\s*保管が義務"),
    ...
    ("N19", _both(r"(C\s*種|D\s*種).{0,40}500\s*" + _OHM, r"(30\s*mA|0\.1\s*秒)"),
    ...
    ("N20", _both(r"(メガ|絶縁抵抗計|測定電圧|印加電圧)", r"1000\s*V.{0,30}解釈\s*第?\s*14\s*条"),
    ...
    ("N21", re.compile(r"Zone\s*1.{0,60}Ex\s*n"),
    ...
    ("N22", re.compile(r"(T5\s*\|\s*100\s*℃\s*\|[^|\n]*(二硫化炭素|CS)|亜硫化炭素)"),
    ...
    ("N23", re.compile(r"(主任技術者|特別高圧|選任).{0,30}常駐(義務|が必要|を要)"
    ...
    ("N25", re.compile(r"原単位.{0,40}1\s*(?:%|％|パーセント).{0,40}義務"),
    ...
    ("N26", re.compile(r"契約電力\*{0,2}\s*[=＝]\s*\*{0,2}その月の最大需要電力"),
     "実量制の契約電力は当月を含む過去1年間（当月＋前11か月）の各月の最大需要電力の最大値。"
     "その月の値だけで決まるのではない",
     "- **契約電力** = その月の最大需要電力（または契約上限）"),
    ("N27", re.compile(r"(デマンド|契約電力|最大需要電力).{0,40}みなし契約"),
    ...
    ("N28", re.compile(r"省エネ法第\s*9\s*条.{0,40}(計量|記録)"),
    ...
    ("N29", re.compile(r"むだ時間.{0,60}(D\s*成分|微分).{0,10}追加"),
    ...
    ("N30", re.compile(r"第\s*339\s*条\s*(?:[〜～~\-–]|から第?)\s*3(?:4[0-9]|5[0-9])\s*条"),
    ...
    ("N31", re.compile(r"1910\.147.{0,30}国際(標準|規格)"),
    ...
    ("N32", re.compile(r"OCR.{0,20}動作.{0,20}地絡"),
    ...
    ("N33", re.compile(r"温度クラス(?:(?!最高表面温度).){0,40}発火温度より低い"),
