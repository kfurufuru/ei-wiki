...
| ループ電源 | Loop Power | — | DCS・レシーバーから 24V を伝送器に供給する方式 |
...
| 電路 | Electric Circuits / Wiring Route | — | ケーブルが通る物理的経路（ラック・管路） |
...
| 安全完全性レベル | Safety Integrity Level | SIL | SIS の信頼性・安全性のレベル（SIL 1〜4） |
...
| NAMUR | NAMUR | — | 計装機器標準化団体（ドイツ）。NE 勧告で信号規格を定める |
| ループ図 | Loop Diagram | — | 1 制御ループの接続を詳細に示した図面 |
| 校正 | Calibration | — | 測定器の指示値を基準に合わせる作業 |
| ゼロトリム | Zero Trim | — | 入力がレンジ下限（0%）のとき出力を 4mA に合わせる調整（下限値は必ずしも 0 ではない） |
| スパン | Span | — | 測定範囲の最大値 - 最小値 |
| ドリフト | Drift | — | 時間経過による指示値のずれ |
| 電流ループ | Current Loop | — | 4-20mA を使ったアナログ信号伝送方式 |
...
| 温度クラス | Temperature Class | T1〜T6 | 機器表面の最高許容温度で分類（T1=450℃〜T6=85℃。数字が大きいほど許容温度が低く、より発火温度の低いガスに対応） |
