...
!!! danger "放電試験・切替試験が判定NGの場合（バックアップ時間不足・切替失敗）"
    試験で基準を満たさなかった場合は、放置せず以下の順で対応する（具体的な連絡先・時限は社内規定に従う）。

    1. **即時報告・代替電源確保**：速やかに電気担当へ報告し、重要負荷（DCS・安全計装等）の代替電源または縮退運用を確保する。
    2. **是正計画の立案**：バッテリー交換または UPS 修理の計画を立て、必要部材・工期を確認する。
    3. **暫定運用条件の明示**：再試験までの間、確保できるバックアップ時間の範囲・監視強化・フェールセーフ確認等の暫定運用条件を関係者へ周知する。
    4. **再試験・記録**：是正後に再試験を実施し、判定結果を点検記録へ残す。
