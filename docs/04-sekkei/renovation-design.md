...
!!! danger "既設盤の盤内温度超過は他の機器の寿命を大幅に縮める"
    電子部品の寿命はアレニウス則に従って温度が高いほど短くなり、電解コンデンサ等では「温度10°C上昇で寿命が概ね半分」という経験則が目安として広く用いられる（実際の倍率は部品や活性化エネルギーにより異なるため、具体的な寿命はメーカーの寿命データを参照する）。盤内温度が設計値を超えた状態で機器を追加するとインバータ・PLCが予定より早く故障する。
...
| 接地種別 | 適用場所 | 接地抵抗基準 | 根拠 |
|---------|---------|------------|------|
| 第3種（D種） | 300V以下の低圧機器外箱 | 100 Ω 以下（0.5秒以内自動遮断で500Ω以下） | 電技解釈第17条 |
| 特別第3種（C種） | 300V超の低圧機器外箱 | 10 Ω 以下（0.5秒以内自動遮断で500Ω以下） | 電技解釈第17条 |
| 計装アース（IA） | 計装機器シールド | 10 Ω 以下（推奨） | JIS C 1010等 |
| A種（高圧機器接地） | 変圧器・遮断器外箱 | 10 Ω 以下 | 電技解釈第17条 |
...
!!! warning "「軽微な変更」でMOCを省略すると法定点検で指摘される"
    電気事業法の保安規程（電気事業法第42条）では変更工事の記録義務がある。年次自主検査や保安監督でのMOC未登録は指摘事項になる。
