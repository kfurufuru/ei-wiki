...
電動機始動時は始動電流での電圧降下も別途確認する。直入れ（DOL）始動では全負荷電流の概ね 5〜8 倍が流れる。Y-Δ・ソフトスタータ・インバータ始動ではこの値は低減する。
...
<p style="font-size:0.78rem;color:#888;margin-top:0.5rem">※ CV 3芯 600V 基準。温度・多条補正係数は別途適用すること。リアクタンス X = 0.09 Ω/km（固定値）は小〜中サイズ CV・力率 0.8 前後での近似値で、この範囲では誤差は小さい。大サイズ・低力率ではケーブルカタログの X 値で再計算すること。</p>
</div>
