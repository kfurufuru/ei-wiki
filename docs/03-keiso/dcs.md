...
!!! warning "緊急変更時の承認・記録フロー"
    緊急でMOC書面化を待てない場合でも、承認と記録の順序を省略しない。

    1. **実施前**：運転責任者（当直長など運転を統括する立場）へ口頭で承認を得る。承認者・時刻・変更内容を運転日誌等に即時メモする。
    2. **実施後**：速やかに（社内規定の期限に従い）MOC を書面化し、承認者と確認者を記録する。
    3. 書面化した MOC は通常の変更と同じルートで正式承認まで進める。事後承認をもって「記録済み」とせず、書面化・確認者記録まで完了させる。
