...
!!! tip "温度上昇が許容値を超える場合"
    上式の ΔT を求め、「盤内到達温度＝設置環境の最高周囲温度 ＋ ΔT」が盤内機器の許容周囲温度を超えるかで対策を分岐する。

    - **超えるが吸気温度に余裕がある場合**：換気ファンを設ける（強制対流で放熱係数が向上し、外気を取り込めるため）。
    - **ファンでも不足する場合**（屋外盤の夏季・高温室など、吸気温度自体が高く外気導入では下がらない場合）：空冷式クーラー（盤用エアコン）を選定する。
...
> 参照: IP 保護等級（IPコード）は JIS C 0920（IEC 60529）による。
