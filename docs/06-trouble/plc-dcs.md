...
!!! warning "バッテリー低下アラームは早急に対応する"
    ...
    バックアップ保持時間は機種により異なるため、まず機種のバックアップ保持仕様を確認し、それより十分早く交換すること。
    目安として、保持仕様の余裕を見てアラーム発生後 1〜2 週間以内の交換を推奨（保持時間が短い機種ではより早く対応する）。
...
```mermaid
flowchart TD
    A[CPU 異常発生] --> B{待機系 CPU<br/>正常?}
    B -->|NO| C[切替中止<br/>メーカーサポートへ連絡]
    B -->|YES| D[製造担当に通知し<br/>実施許可を得る]
    D --> E[待機系への切替操作]
    E --> F{全 I/O<br/>正常?}
    F -->|NO| C
    F -->|YES| G[切替完了<br/>異常系 CPU を切り離し原因調査]
```

*待機系が正常でなければ切替を行わず、切替後に I/O 異常があればメーカーサポートへ連絡する。*
