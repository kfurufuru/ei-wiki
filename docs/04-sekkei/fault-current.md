...
- **遮断容量確認の法的根拠**：電技省令第14条（過電流からの電線及び電気機械器具の保護対策）を受けて、**電気設備の技術基準の解釈 第33条第1項**が「低圧電路に施設する過電流遮断器は、これを施設する箇所を**通過する短絡電流を遮断する能力**を有するものであること」と定めています（高圧・特別高圧は**第34条第一号**に同旨）。本ページの「計算値 ≤ 定格遮断電流」の確認は、この条文の要求そのものです。
- 第33条第1項の**ただし書き**は、通過短絡電流が 10,000 A を超える箇所で、電源側の過電流遮断器と協調させて遮断する方式（いわゆるカスケード保護・バックアップ保護）を条件付きで認めています。変圧器直近で限流ヒューズと組み合わせる実務は、この規定の運用です。
- 上記の条文は **電気設備の技術基準の解釈 令和7年11月版・電技省令（e-Gov 現行条文）で 2026-07-30 に逐語照合**済みです。改訂で番号・細部が変わり得るため、適用時は最新版を確認してください。
- **IEC 60909-0（波高率 κ の式・電動機の拘束回転子電流比 既定値 5）**：κ = 1.02 + 0.98・e^(−3R/X) と既定値 5 は IEC 60909-0:2016（Ed. 2.0）の内容として複数の二次資料で一致確認しました（2026-07-30。**規格原本は未照合**）。なお IEC 60909-0 は **2026 年に新版（IEC 60909-0:2026）に置き換えられています**（IEC Webstore で 2026-07-30 確認。2016 年版は 2026-07-23 に廃止）。新版で κ 式・既定値に変更が無いかは未確認のため、規格準拠の厳密計算では最新版原本を参照してください。
- **計算式・計算例**：%Z = Z・Pb / Vb² × 100、Is3 = Ib × 100 / %Z合計、τ = (X/R) / (2πf) の次元・導出、およびページ内の数値例（52.5 kA・κ ≒ 1.75・i_p ≒ 145 kA、κ 換算表）は 2026-07-30 に再計算で検算済みです。
- **誘導機拠出 4〜6 倍・X/R 典型値・「低圧 MCCB は 25〜50 kA が多い」**：いずれも実務目安・製品動向であり、規格・法令の規定値ではありません。電動機拠出は本来、機器ごとの拘束電流と減衰時定数によります。厳密な検討が必要な系統（大容量同期機がある等）では IEC 60909 準拠の計算またはメーカー解析によってください。
- **系統インピーダンス**：電力会社回答値・変圧器銘板 %Z・ケーブルカタログ値が一次情報です。銘板 %Z は自己容量基準のため、基準容量への換算を忘れると桁を誤ります。
...
- 電気設備に関する技術基準を定める省令（平成9年通商産業省令第52号）: [第14条](https://laws.e-gov.go.jp/law/409M50000400052#Mp-Ch_1-Se_3-Ss_2-At_14)
- 電気設備の技術基準の解釈（令和7年11月20日改正）: [経済産業省 原典 PDF](https://www.meti.go.jp/policy/safety_security/industrial_safety/law/files/dengikaishaku.pdf) — 第33条。**2026-09-18 に同 PDF の該当条を直接照合**し、本ページの引用内容と一致することを確認しました（前回照合 2026-07-30。条番号の異同はありません）
