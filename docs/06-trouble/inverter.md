...
flowchart TD
    A[OC 発生] --> B{いつ出たか}
    B -->|加速中| C[加速時間が短い<br/>負荷イナーシャ大]
    B -->|起動直後
低速時| D[トルク不足<br/>V/f 設定不適<br/>負荷ロック]
    B -->|定常運転中| E[過負荷<br/>軸受ロック<br/>相欠け]
    C --> C2[加速時間を延長<br/>トルクブースト見直し]
    D --> D2[低速トルクブースト増<br/>空転確認]
    E --> E2[3相電流を測定<br/>軸受温度・振動確認]
```

*OC は「いつ出たか」で原因が分かれる。まず発生タイミングを特定してから下の各タブで詳細を確認する。*
