...
description: "電動機を含む低圧幹線の許容電流（太さ）と過電流遮断器（MCCB）定格電流の求めかた・電技解釈第148条の計算式と計算例"
tags:
  - 設計
  - 低圧
  - ケーブル
...
電動機の合計 ΣI_M と、その他負荷の合計 ΣI_H の大小で場合分けする。**この場合分けと係数 1.25 / 1.1・50 A の境界は電気設備の技術基準の解釈 第148条第1項第二号イ・ロ**が定めるもので（2026-09-18 に原典 PDF で逐語照合）、内線規程が独自に定めた値ではありません。
...
Step 2: 幹線の許容電流（太さ）
  ...
  I_W ≥ 1.1 × 70 + 20 = 97 A
  → CV 3芯 ケーブルラック単条で 97 A 以上 = 38 mm²（許容 140 A）を選定
     ※ lv-cable.md 許容電流表より。22 mm²（100 A）は補正代を考え不採用

Step 3: 過電流遮断器の定格電流
  (a) 3 × ΣI_M + ΣI_H = 3 × 70 + 20 = 230 A
  (b) 2.5 × I_W      = 2.5 × 140 = 350 A   （実配線 38 mm² の許容電流で計算）
  算定値 = min(230, 350) = 230 A
  I_W = 140 A > 100 A で 230 A は標準定格に無い
    → 第五号ハにより直近上位の 250 A（250AF/250AT）を選定
  （幹線許容電流が 100 A 以下の系統なら、直近下位 225 AT を選定する）
```

!!! tip "(b) の I_W は実際に選んだ電線の許容電流"
    (b) は「必要許容電流 97 A」ではなく、実際に採用した電線（38 mm² = 140 A）の許容電流で計算する。太い電線を選ぶほど遮断器を大きくできる、という関係になる。
...
- 法的根拠：**電気設備技術基準の解釈 第148条（低圧幹線の施設）**、および分岐回路は第149条。**1.25 / 1.1・50 A 境界・遮断器定格の各式はすべて第148条が持つ規定**で、内線規程（JEAC 8001）由来ではありません（同規程は有償・原本未照合のため、本ページは同規程の節番号を引用しません）。
...
- 上記の条文表現は **電気設備の技術基準の解釈 令和7年11月20日改正版の原典 PDF で 2026-09-18 に再照合**しました（初回 2026-07-18。条番号・号の異同なし）。ただし改訂で番号・細部が変わり得るため、適用時は最新版の該当条文を必ず確認すること。
