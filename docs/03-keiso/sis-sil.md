...
### SIL 定義表（低頻度作動要求モード, IEC 61508-1 表2＝JIS C 0508-1 表2）
...
## SIL 検証計算例 — PFDavg を手で当たってみる

SIL 検証は最終的には認証済みの故障率データと専用ツールで行いますが、**桁がどう決まるか**を手計算で押さえておくと、テスト周期や冗長化の効き方を現場で検算できます。ここでは前述の SIL 定義表（低頻度作動要求モード）と整合する形で、簡易近似式による計算例を示します。

### 前提となる近似式（低頻度作動要求モード）

単一チャネル（1oo1）の PFDavg は、危険側未検出故障率 λDU（Dangerous Undetected failure rate, /h）とプルーフテスト間隔 TI（h）を使って次で近似できます（IEC 61508-6（JIS C 0508-6）附属書B の式から修復時間（MRT/MTTR）の項を省いた簡易形）。診断で検出できる危険側故障（λDD）はオンラインで警報が出て早期に修復されるため、待機系の PFDavg にはほとんど効かず、**主に λDU が支配**します。

```text
1oo1:  PFDavg ≒ λDU × TI / 2
```

- TI/2 は「DU が発生してから次のプルーフテストで摘出されるまでの平均潜伏時間」を表します
- PFDavg が TI に比例するため、テスト間隔を延ばすほど直線的に悪化します（前節「なぜ必須か」と同じ関係）

### 数値計算例（1oo1・センサ1本）

| 項目 | 記号 | 値 |
|------|------|-----|
| 危険側未検出故障率 | λDU | 1×10⁻⁶ /h |
| プルーフテスト間隔 | TI | 1年 ＝ 8760 h |
...
この 4.4×10⁻³ は SIL 定義表の **10⁻³ ≤ PFDavg < 10⁻²＝SIL 2 帯**に入ります。RRF（＝1/PFDavg）に直すと約 230 で、SIL 2 の RRF 帯（100〜1,000）とも整合します。

- **感度**: テスト間隔を半年（4380 h）に短縮すると PFDavg ≒ 1×10⁻⁶ × 4380 / 2 ≒ 2.2×10⁻³ となり、SIL 2 帯の中で約半分に改善します。テスト頻度が PFDavg に直接効くことが数字で見えます

### 投票構成による改善（1oo2）と共通原因故障

同じセンサを2本にして 1oo2（2本中1本の検知で作動）にすると、危険側に失敗するのは**両方が同時に DU 状態になったとき**だけになるため、独立故障だけを見れば PFDavg は大きく下がります。ただし実際には、両チャネルが同一原因で同時に壊れる**共通原因故障（CCF: Common Cause Failure）** が下限を支配します。CCF は β 係数（全 DU のうち共通原因で失われる割合）でモデル化します（IEC 61508-6 の βファクタ法。「典型 1〜10 %」は二次資料で広く使われる目安であり、規格が定める固定値ではありません）。
...
上と同じ λDU＝1×10⁻⁶ /h、TI＝8760 h、β＝5 % で計算すると次のようになります。

| 項 | 計算 | 値 |
|----|------|-----|
| 独立故障項 (λDU×TI)²/3 | (8.76×10⁻³)² / 3 | ≒ 2.6×10⁻⁵ |
| CCF項 β×λDU×TI/2 | 0.05 × 8.76×10⁻³ / 2 | ≒ 2.2×10⁻⁴ |
| 合計 PFDavg | 2.6×10⁻⁵ ＋ 2.2×10⁻⁴ | ≒ 2.4×10⁻⁴ |

- 1oo1 の 4.4×10⁻³（SIL 2 帯）から **2.4×10⁻⁴（10⁻⁴ ≤ PFDavg < 10⁻³＝SIL 3 帯）** へ、約1桁改善しました
- ただし合計を支配しているのは独立故障項ではなく **CCF項**です。β を下げられなければ、いくら冗長化しても PFDavg はこの CCF 項より下がりません。**CCF 低減（別メーカー・別測定原理・物理分離・ダイバーシティ）が冗長化の効果を決める**、というのが 1oo2 の要点です
- **2oo3** は危険側にもスプリアス側にも1個の故障を許容でき（前述の投票構成表・HFT=1 相当）、安全性と運転継続性を両立させる構成として実務で多用されます

### SIF トータルの積み上げ

SIF の PFDavg は、センサ・ロジックソルバ・最終要素それぞれの PFDavg の**足し算**で評価します（SIF 節と同じ考え方）。目標 SIL の PFDavg 帯に合計が収まるかを照合します。

| 要素 | 構成・前提（例） | PFDavg（例） |
|------|-----------------|--------------|
| センサ | 1oo2・上記計算 | 2.4×10⁻⁴ |
| ロジックソルバ | 安全 PLC（IEC 61508 認証品） | 1.0×10⁻⁴ |
| 最終要素（ESDV） | 1oo1・PST 併用でフルテスト間隔を延伸 | 2.0×10⁻³ |
| **SIF 合計** | 単純加算 | **≒ 2.3×10⁻³ → SIL 2 帯** |

- 合計 2.3×10⁻³ は SIL 2 帯（10⁻³ ≤ PFDavg < 10⁻²）に収まり、この SIF は **SIL 2 を満たす**と判断できます
- 内訳を見ると **最終要素（弁）の寄与が約 9 割**で支配的です。弁は可動部の固着という DU を抱えやすく、故障率が大きくなりがちなためで、SIF 節の「最終要素が支配的」と一致します。PFDavg を下げたいときはまず**弁に手を打つ**（PST 併用でカバレッジを上げる・冗長化する・フルテスト周期を見直す）のが効果的です
- 目標 SIL に届かなければ、**テスト間隔 TI 短縮 → 冗長化（投票構成変更）→ PST 導入**の順で対策を回し、再計算して帯に収まるかを確認する、という設計ループになります

!!! warning "この計算例は「桁を掴む」ための簡易版です"
    ここで使った近似式は、故障率一定・TI ≫ 修復時間・カバレッジ 100 % などの理想化を前提にした概算です。**正式な SIL 検証は、認証済みの故障率データ（SIL データシート／exida 等の第三者認証データ）と、CCF・診断カバレッジ・部分テストを織り込んだ専用ツールで実施**します。本例はあくまで「設計者が桁感を持ち、ツールの出力を検算し、テスト周期や冗長化の効き方を現場で説明する」ための道具立てであり、この手計算だけで SIL を主張するものではありません。次節のアーキテクチャ制約（SFF・HFT）も併せて満たす必要があります。
...
- **SIL の定義帯（PFDavg・PFH）**: **2026-09-19 に無償の一次資料で確定しました。** 従来は JIS C 0508-1:2012 表2・表3（有償・原本未照合）だけを挙げていましたが、同じ帯が**厚生労働省告示**にあり、全数一致を確認しました。

    出典: [機能安全による機械等に係る安全確保に関する技術上の指針（平成28年9月26日 厚生労働省告示第353号）](https://www.mhlw.go.jp/file/06-Seisakujouhou-11300000-Roudoukijunkyokuanzeneiseibu/0000140170.pdf) 別紙（2026-09-19 に原典 PDF で照合）。同指針は IEC 61508（JIS C 0508 系）の安全度水準を国内の技術上の指針として採り入れたものです。

    | 要求安全度水準 | 低頻度作動要求モード（PFDavg） | 高頻度の作業要求モード又は連続モード（PFH・1/h） |
    |---|---|---|
    ...
    **モードの境界も同告示が定義しています。** 低頻度作動要求モードは「要求安全機能の作動が求められる頻度が **1 年当たり 1 回以下**」、高頻度は「1 年当たり 1 回を超える」、連続モードは「通常運転の一環として作動が連続的に求められる」場合です（注1〜注3）。**どちらの表を使うかはこの頻度で決まります。**JIS C 0508-1 は IEC 61508-1:2010 の対応規格。**原本未照合・二次資料のみ**（2026-07-30 の照合は非公式な全文転載サイトによるもので、出典として扱いません。表番号・帯域の数値は JSA/IEC 発行の原本で確認してから引用してください）。
- **PFDavg の近似式**: JIS C 0508-6:2019 附属書B（1oo1 の式は PFDG＝(λDD＋λDU)×tCE、tCE＝T1/2＋MRT）は**原本未照合・二次資料のみ**（附属書番号・式は原本で未確認。以前の確認は非公式な全文転載サイトによるもので、出典として扱いません）。本文の「λDU×TI/2」「(λDU×TI)²/3＋β×λDU×TI/2」は**修復時間・診断修復を無視した簡易形**であり、規格の式そのものではない（桁の把握用。本文の警告どおり、この式だけで SIL を主張しないこと）。
- **BPCS と SIS の独立・LOPA・アーキテクチャ制約（SFF/HFT）**: IEC 61511（プロセス産業向け。JIS C 0511 系）・IEC 61508-2 に基づく一般的な設計原則だが、**該当する条文・表の番号までは原本未照合**のため本ページでは条番号を挙げない。SFF の閾値（60 %／90 % など）も二次資料で広く使われる区分の例示である。適用時は規格原本と認証機関・SIL 検証の担当者に確認すること。
- **β＝1〜10 %・λDU・故障率・PFDavg の各数値例**: すべて教育目的の例示であり、規格の規定値でも実機の代表値でもない。実際の SIL 検証は認証済み故障率データと専用ツールで行う。
- **限界の明示**: 本ページは SIS/SIL の考え方の整理であり、**この記載のみを根拠に SIL の判定・プルーフテスト周期の変更・投票構成の変更を行ってはならない**。それらは SIL 検証計算と MOC の対象である。
...
- [アラーム管理](alarm-management.md) — LOPA でオペレータ介入を独立防護層として成立させる土台（SIS の1つ手前の層）
