...
!!! warning "瞬時整定を定格の1.5〜2倍に置いてはいけない"
    瞬時要素を定格の 1.5〜2 倍（150〜200%）に整定すると、変圧器投入のたびに励磁突入電流（定格の 6〜10 倍）で瞬時要素が誤動作トリップします。瞬時要素は励磁突入電流の上を確実にまたぐ値に設定してください（[保護継電器](relay.md) の整定計算例と整合）。<!-- lint-ok: 20260710-ocr-shunji-150pct 誤りの説明のため -->
