...
!!! abstract "まず30秒でやること"
    1. **人の安全確保**（負傷者対応・周囲の安全確認）
    2. 危険があれば**遮断・避難**（迷ったら過剰対応でよい）
    3. 2分以内に**製造課へ第一報**（原因不明でも「調査中」と伝える）
...
!!! note "初動の役割分担と報告経路"
    複数人いる場合は、**通報担当／現場対応担当／記録担当**を早期に分担する（1人のときは安全確保を最優先に順次実施）。

    - **怪我・火災・漏洩がある場合**：119番・保安室・上長への連絡は**並行**で行う（1人が現場対応、別の1人が通報）。
    - **設備トラブル（生産影響）の場合**：製造課当直 → 電計チームリーダー の順で**順次**報告する。

    （連絡先の詳細は本ページ末尾「緊急連絡系統」を参照）
...
- [ ] 安全上のリスクがある（感電・爆発・漏洩）→ 時間の目安を待たず即エスカレーション
- [ ] **異常検知時刻を起点として30分以内**に原因特定ができない
