...
単一チャネル（1oo1）の PFDavg は、危険側未検出故障率 λDU（Dangerous Undetected failure rate, /h）とプルーフテスト間隔 TI（h）を使って次で近似できます（IEC 61508-6）。診断で検出できる危険側故障（λDD）はオンラインで警報が出て早期に修復されるため、待機系の PFDavg にはほとんど効かず、**主に λDU が支配**します。
...
同じセンサを2本にして 1oo2（2本中1本の検知で作動）にすると、危険側に失敗するのは**両方が同時に DU 状態になったとき**だけになるため、独立故障だけを見れば PFDavg は大きく下がります。ただし実際には、両チャネルが同一原因で同時に壊れる**共通原因故障（CCF: Common Cause Failure）** が下限を支配します。CCF は β 係数（全 DU のうち共通原因で失われる割合、典型 1〜10 %）でモデル化します（IEC 61508-6）。
