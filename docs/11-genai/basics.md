...
```mermaid
flowchart LR
    A["入力文<br/>「今日の天気は」"] --> B{"次に来る言葉の候補"}
    B --> C["晴れです"]
    B --> D["良いですね"]
    B --> E["…"]
    C --> F["最も確率が高い<br/>言葉を選ぶ"]
    D --> F
    E --> F
    F --> G["つなげて<br/>文章を生成"]
```

*入力に続く言葉の候補から、確率が高いものを選んでつなげていく。これを繰り返すのが「次の言葉を予測し続けるエンジン」。*
...
| ツール | 提供元 | 使える場面 | 特徴 |
|--------|--------|-----------|------|
| ChatGPT | OpenAI（米国） | ブラウザ・スマホアプリ | 最も普及。日本語対応。無料版でも標準的なモデルが使える（提供モデルは随時更新。最新は公式ページで確認） |
...
| 正確な数値計算 | 簡単な計算はできることが多いが、桁数が多い計算や複雑な計算は誤ることがある（コード実行機能付きなら精度向上）。数値は必ず別途検算する |
...
3. 無料プランで標準的なモデルが使える（提供モデルは随時更新。最新は公式ページで確認）
4. 有料プラン（ChatGPT Plus, 月額約20ドル。2026-04時点。最新は公式料金ページで確認）でより高性能なモデルが使用可能
