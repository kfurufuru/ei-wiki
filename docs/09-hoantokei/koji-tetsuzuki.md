...
!!! warning "施工フェーズは労安が主役"
    このフェーズで発生するのは主に労働安全衛生法上の手続き（作業許可・特別教育・停電作業）です。感電・電気火災などの事故が起きた場合の国への報告は、電気事業法第106条の報告徴収を受けた**電気関係報告規則**の側で定まります。対応フローは [電気事故対応と報告](jiko-taiou.md) にまとめています。

## フェーズ⑤: 検査・試運転 { #p5 }

使用開始の前に通すべき検査です。

- **使用前自主検査**: 工事計画届出（法第48条第1項）をして設置・変更の工事をする事業用電気工作物のうち**主務省令で定めるもの**は、使用開始前に設置者自身が自主検査を行い、結果を記録・保存します（電気事業法**第51条第1項**）。検査では、工事が届出した計画に従って行われたことと、技術基準への適合を確認します（同条第2項）。
- **使用前安全管理審査**: 使用前自主検査の**実施に係る体制**（組織・検査の方法・工程管理など）が適切かを、登録を受けた審査機関等が審査する仕組みです（同条**第3項・第4項**。条文の見出しは「使用前安全管理検査」）。
- 対象の細目（主務省令が定める範囲・審査の時期）は本ページには書きません。要否の見立ては [法定業務一覧](legal-duties.md) の工事計画届出・使用前検査の項と所轄の産業保安監督部の案内で確認してください。

| 手続き・確認 | 実施者 | 目安時期 | 根拠法令 | 詳細リンク |
|-------------|-------|---------|---------|-----------|
| 使用前自主検査・使用前安全管理審査 | 電気主任 | 使用開始前 | 電気事業法第51条 | [法定業務一覧](legal-duties.md) |
...
本ページはハブであり、各手続きの様式・数値・条文解説の正典はリンク先の各ページが持ちます。本ページ自身が言及する条文と、正典への割り当ては以下のとおりです。

- **使用前自主検査・使用前安全管理審査（フェーズ⑤）**: 電気事業法（昭和39年法律第170号）**第51条第1項〜第4項**。**e-Gov 法令検索の現行条文で 2026-08-01 に逐語照合**し、従来「条番号要確認」としていた保留を解消しました。第1項の対象は「第48条第1項の届出をして設置又は変更の工事をする事業用電気工作物であって主務省令で定めるもの」であり、**対象の細目を定める主務省令（施行規則の該当条）は未照合**のため本ページには書いていません。
- **工事計画届出・保安規程・国への事故報告**: [法定業務一覧](legal-duties.md)（法第42条・第43条・第48条・第106条、施行規則第65条・別表第二）と [電気事故対応と報告](jiko-taiou.md)（電気関係報告規則第3条）の根拠節が正典です。
- **高圧ガス保安法・消防法**: [規格・法規](../04-sekkei/standards.md) の根拠節が正典です（高圧ガスの変更は第一種製造者＝**許可**・第二種製造者＝事前届出・軽微変更＝事後届出の区分、消防法の変更許可・完成検査、防爆の委任チェーンを含む）。
- **労働安全衛生法（フェーズ④）**: 特別教育は [特別教育・資格一覧](../10-safety/special-education.md)（労安法第59条第3項・安衛則第36条第四号）、停電作業は [停電作業](../05-hozen/outage-work.md)（安衛則第339条）の根拠節が正典です。
- **電気工事士法・電気工事業法の資格区分（フェーズ③）**: [制御盤撤去ガイドライン](../guidelines/seigyoban-tekkyo-guideline.md) を参照します（本ページでは条文照合していません）。

**限界の明示**:

- フェーズ見出しの時期（「着工6か月前〜」「着工1〜3か月前」）とリードタイムの図は**実務上の目安**であり、法定の期限ではありません。法定の待機期間・提出期限は正典ページの条文で確認してください。
- 「電力会社手続き」列は法令ではなく契約・運用上の手続きです（本文 note のとおり）。
- マトリクスの○の付け方（どのフェーズでどの法令が動くか）は一般的な進め方の整理であり、個別の工事では前後・重複し得ます。
...
- 電気事業法（昭和39年法律第170号）: [第51条](https://laws.e-gov.go.jp/law/339AC0000000170#Mp-Ch_3-Se_2-Ss_4-At_51)・[第48条](https://laws.e-gov.go.jp/law/339AC0000000170#Mp-Ch_3-Se_2-Ss_4-At_48)
