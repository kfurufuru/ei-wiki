...
!!! tip "露点の条件は 2 つあります（ISA-7.0.01 由来とされる）"
    加圧露点について、次の**両方**を満たすという考え方が広く用いられています。

    1. **系統がさらされる現地の最低記録周囲温度より少なくとも 10 °C（18 °F）低い**こと
    2. **ライン圧力のもとで 4 °C（39 °F）を超えない**こと

    1 は「夏は問題なくても冬に結露・凍結する」ことを防ぐ最低気温基準、2 はどんな温暖地でも外せない絶対上限です。**本ページは 2026-09-19 まで 1 しか書いていませんでした。**

    !!! danger "冷凍式ドライヤの上限側は 2 を満たしません"
        上の比較表のとおり、冷凍式の到達露点は **加圧露点 +3〜+10 °C** です。上限側の +10 °C は **2 の 4 °C を超えます**。冷凍式を採る場合は、到達露点が 4 °C 以下に収まるか、そもそもこの条件を適用する系統かを設計で確認してください。

    この考え方は計装空気品質の規格 ISA-7.0.01 に由来するとされますが、**同規格は有償で本ページでは原本を照合していません**。上の 2 条件は 2026-09-19 に業界誌の解説記事で確認した**二次資料ベース**です（下記「根拠」参照）。調達仕様・受入判定に使う際は規格原本または自工場のユーティリティ仕様書を正としてください。

### 油分・粉塵

- **油分**: ポジショナのノズル・フラッパ機構やダイヤフラムに付着し、固着・動作不良の原因になります。一般的な目安として 1 ppm 程度以下に管理されます（**液体・蒸気の合計で 1 ppm (w/w) 以下**が ISA-7.0.01 の要求とされます。二次資料ベース）
- **粉塵**: 絞り部・パイロット弁の詰まりの原因。多くの空気式機器では粒子径 40 μm 以下が一般的な目安とされます（同じく ISA-7.0.01 の**最大粒子径 40 μm** に対応するとされます。より微細な管理が必要な機器もあります）
- **腐食性・可燃性・毒性の混入物が無いこと**も同規格の要求項目とされます（数値ではなく定性要求）。窒素バックアップやプロセスガスの逆流がありうる系統では、この観点も確認してください

### 規格と「自工場の仕様」の関係

- **ISA-7.0.01**: 計装空気の品質規格。**有償で原本は未照合**のままです。2026-09-19 に業界誌の解説記事（[Compressed Air Best Practices — Understanding the ISA-7.0.01-1996 Instrument Air Standard](https://www.airbestpractices.com/standards/iso-and-cagi/understanding-isa70011996-instrument-air-standard)）で 4 項目の内容を確認し、**露点の条件が 2 つある**こと（最低周囲温度 −10 °C かつライン圧で 4 °C 以下）と、粒子径 40 μm・油分 1 ppm (w/w)・腐食性等の混入物が無いこと、を本文に反映しました。**これは二次資料であり、規格原本ではありません。** 調達仕様・受入判定の数値は規格原本で確認してください（同規格は 1996 年以降改正されていないとされ、実務では ISO 8573-1 のほうが厳しく適用されることも同記事が指摘しています）
- **JIS B 8392-1（ISO 8573-1）**: 圧縮空気の清浄等級。固体粒子・水分（加圧露点）・油分の3項目について等級（クラス）が定められているとされます（**有償・原本未照合。版年も未確認**）
...
計装空気の圧力・品質の目安値は**本ページを正典（SSOT）**とし、他ページは再記述せず本ページを参照します。対象は次のとおりです。

| 値 | 位置づけ |
|---|---|
| ヘッダ圧 0.4〜0.7 MPa | 実務慣行の代表レンジ |
| 末端の空気式操作端 概ね 0.14 MPa 以上 | 弁により異なる。**弁データシートが正** |
| 油分 1 ppm 程度以下 | 実務慣行の目安 |
| 粉塵 粒子径 40 μm 以下 | 多くの空気式機器での一般的な目安 |
| 加圧露点 冷凍式 +3〜+10 °C／吸着式 −20 °C 以下（−40 °C 級が一般的） | メーカー仕様の代表値 |

**いずれも法令・JIS の規定値ではありません。**確定値は自工場のユーティリティ仕様書・基本設計書・ドライヤ購入仕様書によります。

### 規格（いずれも有償・原本未照合）

- **ISA-7.0.01**（計装空気の品質規格）: 「加圧露点を現地の最低記録周囲温度より少なくとも 10 °C 低くする」という考え方の由来とされます。**有償規格で原本を照合していません**（複数の二次資料で一致することを 2026-08-01 に確認したにとどまります）。このため**節番号・条項番号は挙げず、要求文言の逐語引用もしません**。
- **JIS B 8392-1（ISO 8573-1）**（圧縮空気の清浄等級）: 固体粒子・水分・油分の3項目で等級を定めるとされます。**有償規格で原本未照合・版年も未確認**のため、等級の具体値（クラス番号と対応する数値）は本ページに記載していません。清浄等級を調達仕様に使う場合は規格原本を参照してください。

### 法令

計装空気そのものの圧力・品質を直接規定する法令は、本ページの範囲では確認していません。関連する法令要求は次のページが正典です。

- 防爆エリアでの機器選定（Zone・防爆構造）→ [防爆](explosion-proof.md)
- 高圧ガス設備での事故届・変更手続き → [制御弁トラブル](../06-trouble/valve.md)・[規格・法規](../04-sekkei/standards.md)

### 限界の明示

- レシーバの保有時間・優先供給ロジック・バックアップ構成は**工場ごとの設計**であり、本ページは一般的な考え方のみを示します
- 点検頻度（日常〜週次等）は一般的な目安で、法定の点検周期ではありません
- フェールポジション（FC/FO/FL）の個々の弁の向きは**設計で決まる値**であり、本ページの表は区分の一般的な整理です。必ず P&ID・弁データシートで確認してください
