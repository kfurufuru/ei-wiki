...
!!! warning "直流耐圧の試験電圧はページ間で値が異なる — 要確認"
    上式は一例・参考値。[耐圧試験・絶縁診断](insulation-test.md) では 6.6kV ケーブル竣工時に **22〜25kV 直流** の例を挙げており、本ページの参考値と大きく異なる。試験電圧は適用規格（JCS／JEAC 等）と試験目的（竣工／保守）で変わるため、実施前に必ず該当規格・社内基準で確認すること。なお経年 CV ケーブルへの直流耐圧は水トリー劣化を進める懸念があり、実施可否自体も要検討。
