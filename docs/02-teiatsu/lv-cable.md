...
e     : 電圧降下 [V]
I     : 電流 [A]
R     : 導体抵抗 [Ω/km]（90 ℃ の交流実効抵抗 R(90℃)。カタログの 20 ℃ 値は使わない）
X     : リアクタンス [Ω/km]（サイズ・種別・周波数で決まる。R・X とも [正典の表](../04-sekkei/voltage-drop.md)参照）
...
- **導体抵抗 R** は **90 ℃ の交流実効抵抗 R(90℃)**（技資第103号B 表③・表④の「R(90℃)」列）で電圧降下を計算します。2026-09-17 に原本 PDF で全サイズ・両周波数を照合し、[電圧降下計算](../04-sekkei/voltage-drop.md) の R 表と同一値です（照合経路は同ページの「根拠」が正典）。**単心は別値**で、本ツールは扱いません
- 2026-08-02〜09-17 の本ツールは JIS C 3605 の 20 ℃ 値で計算しており、電圧降下を 2 割強小さく出していました。その期間にこのツールで判定した回路は再計算してください。20 ℃ 値は `CABLE_DB` にカタログ照合用の参考値として残していますが、計算には使いません
