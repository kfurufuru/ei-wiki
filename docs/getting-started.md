...
経験レベル別に「何から読むか」を示す**ナビゲーション（索引）**です。技術的な数値・基準は**すべてリンク先の各正典ページが持ち**、本ページは値を独自に定義しません（現状、本ページに数値記述はありません）。読む順番の区切り（3ヶ月・Step 1〜3 等）は学習計画の**目安**であって、資格・教育の法定要件ではありません。

### 是正の記録（2026-08-06）

各カードの説明文を実際のページ内容と突き合わせた結果、**数値のずれは無し**（そもそも数値記述なし）。説明文の不一致を1件是正しました。

| 箇所 | 旧記述 | 是正後 |
|---|---|---|
| 保全体系の概要 | 「**PM**・CBM・BM の違い」 | [保全体系の概要](05-hozen/maintenance-system.md)本体は保全方式を **BM/TBM/CBM/PdM** の区分で解説しており、「PM」という括りの節は無い。ページの用語体系に合わせた |

リンク先の実在は全リンク分を確認済み（`mkdocs build --strict` 通過）。

### 限界の明示

- **本ページに載っていないページは「読まなくてよいページ」ではありません。** 各カテゴリの全記事は各カテゴリの index（例: [計装](03-keiso/index.md)・[高圧](01-koatsu/index.md)・[低圧](02-teiatsu/index.md)・[安全管理](10-safety/index.md)）から辿ってください
- 読む順番・期間の目安は標準的な OJT を想定した推奨であり、個人の経験・配属業務によって適切な順番は変わります

照合日: 2026-08-06（repo 内の各リンク先と説明文を突き合わせ。索引ページ自体は独自の一次照合を持ちません）。
