...
```
竣工時の直流耐圧試験電圧 ＝ 最大使用電圧 × 1.5（交流試験電圧）× 2
  最大使用電圧 ＝ 公称電圧 × 1.15 / 1.1 （6.6kV 系 → 約 6.9 kV）
例：6.6kV 系 → 6.9 × 1.5 × 2 ≒ 20.7 kV 直流（10 分間印加）
```

!!! note "試験電圧の求め方（竣工と保守で異なる・電技解釈第15条）"
    竣工時の直流耐圧は「最大使用電圧 × 1.5（＝交流試験電圧）× 2」で求める（電技解釈第15条：ケーブルは交流試験電圧の 2 倍の直流でも可）。6.6kV 系で約 **20.7kV 直流** となり、[耐圧試験・絶縁診断](insulation-test.md) の竣工 22〜25kV とも整合する。保守（維持）時はこれより低い電圧・短時間で行う。なお経年 CV ケーブルへの直流耐圧は水トリー（水トリー）劣化を進める懸念があり、AC / VLF 耐圧や絶縁診断（tanδ・PI 値）への切替えも検討する。
