...
!!! note "本表の「対策」は法令要求より厳しい実務・社内運用を含む"
    停電作業の法令上の措置（労安則第339条）と実務の対応関係は[停電作業](../05-hozen/outage-work.md)の根拠節が正典。条文では施錠・通電禁止の表示・監視人は**選択制**であり、検電＋短絡接地の義務は**高圧・特別高圧であった電路**に限られる。本表の「毎回検電」「1人1錠」はそれより厳しい安全側の実務水準。
...
| 26 | 危険場所の区分見直し後も旧検定の機器をそのまま使い続けた | 機器の検定内容が現在の危険場所区分に適合していない状態で稼働していた | 防爆機器台帳（検定区分・適合範囲）を整備し、区分変更・機器更新時に適合を再確認 |
...
| 28 | モーター試運転前の絶縁抵抗確認を省略 | 巻線の絶縁不良のまま通電し焼損 | 電動機の通電前は絶縁抵抗を測定（判定値は[絶縁管理](../05-hozen/insulation-management.md)の法定下限・実務管理値による） |
...
```mermaid
pie title ヒヤリハット発生タイミングの目安（経験則・統計データではない）
    "作業開始直後（準備段階の省略）" : 35
    "長時間作業の後半（集中力低下）" : 25
    "作業終了時（復帰確認の省略）" : 25
    "引き継ぎ・交代時（情報伝達ミス）" : 15
...
本ページは「電気計装現場のヒヤリハット**事例集と再発防止の実務**」の正典です。事例は経験則に基づく類型であり、特定の事故事例の記録ではありません。法令上の停電作業の措置は [停電作業](../05-hozen/outage-work.md)、絶縁抵抗の判定値は [絶縁管理](../05-hozen/insulation-management.md)、LOTO の手順は [ロックアウト・タグアウト](lockout-tagout.md) が正典で、本ページは数値・条項を再記述しません。

### 是正の記録（2026-08-06）

- **事例26「防爆認定が切れた旧型機器」→「危険場所の区分見直し後も旧検定の機器を使用」**: 型式検定の合格に設置済み機器の「使用期限」があるかのような書き方と、「更新を怠ったことが法定点検で指摘事項になった」という**一次照合していない法令要求の含意**を撤回。実在するリスク（検定内容と現在の危険場所区分の不適合。[防爆](../03-keiso/explosion-proof.md)の検定系統確認の記述と整合）に書き換えた
- **事例28の対策「絶縁抵抗測定（1MΩ以上を確認）」→ 正典参照へ**: 1 MΩ は低圧電動機の実務管理値で法定値ではなく、高圧電動機は「設備電圧 [kV]＋1 [MΩ] 以上」と判定値が異なる。単一数値の断定をやめ [絶縁管理](../05-hozen/insulation-management.md) 参照に変更
- **円グラフの題を「分布（推計）」→「目安（経験則・統計データではない）」**: 35/25/25/15 の比率は統計調査の結果ではなく執筆時の経験則であることを明示
- **法令要求と実務水準の切り分け注記を追加**: 停電・検電作業の表の対策（毎回検電・1人1錠）が労安則第339条の要求より厳しい実務水準であることを明示（条文対応は[停電作業](../05-hozen/outage-work.md)の根拠節が正典）

### 二次資料で一致を確認（原本未照合）

- **ハインリッヒの法則（1:29:300）**: 出典とされる Heinrich の原著は未照合。安全教育で広く引用される比率として複数の安全解説資料で一致を確認。経験則であり、比率自体に統計的普遍性があるわけではない

### 実務慣行（規格・法令の裏付けなし）

- シールドの片端接地（制御室側）の原則（詳細は[接地・ノイズトラブル](../06-trouble/grounding-noise.md)）
- 強制ON/OFF の台帳管理、PLC変更前バックアップ、HMI の確認ポップアップ、復電前チェックリストの項目立て、2時間目安の休憩、指差し呼称
- 防爆機器の締め付けトルク管理・台帳の年1回見直しの周期

### 限界の明示

- 本ページの30事例は**類型の例示**であり、自事業場のリスクアセスメント・作業手順書を代替しない
- 「対策・チェックポイント」列は最小限の要点であり、実施の詳細（検電器の選定・LOTO の錠管理など）は各正典ページと自事業場の規程による

### 正典参照

| 項目 | 正典 |
|---|---|
| 停電作業の法定措置（労安則第339条の条文対応） | [停電作業](../05-hozen/outage-work.md) |
| 検電を必須とする本Wikiの方針 | [作業許可制度（PTW）](ptw.md) |
| 絶縁抵抗の法定下限・実務管理値 | [絶縁管理](../05-hozen/insulation-management.md) |
| LOTO の手順・錠の管理 | [ロックアウト・タグアウト](lockout-tagout.md) |
| 防爆機器の検定系統・適合確認 | [防爆](../03-keiso/explosion-proof.md) |
| シールド接地・グラウンドループ | [接地・ノイズトラブル](../06-trouble/grounding-noise.md) |

照合日: 2026-08-06（事例中の数値・法令含意を上記正典ページと突き合わせ、一次照合できない法令含意は撤回）。

## 関連ページ

- [リスクアセスメント](risk-assessment.md) — ヒヤリハットを危険源評価につなげる
- [ロックアウト・タグアウト](lockout-tagout.md) — 電気作業の代表的な予防措置
- [安全カテゴリ](index.md) — より広い安全管理へ
