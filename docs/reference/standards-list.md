...
| 規格番号 | タイトル | 適用場面 | 入手先 |
|---------|---------|---------|-------|
| JIS C 0617 | 電気用図記号（IEC 60617 対応） | 展開図・結線図・単線結線図の図記号（**P&ID の計装記号は JIS Z 8204／ISA-5.1 の担当**） | JSA |
| JIS Z 8204 | 計装用記号 | P&ID・計装フロー図のループ番号・機能記号（TT/TIC など）。詳細は [計装工事で押さえるJIS規格](../03-keiso/jis-standards.md) | JSA |
...
| JIS C 0617 の適用場面（2026-08-06） | 「展開図・**P&ID** の図記号」 | **P&ID の計装記号は JIS Z 8204／ISA-5.1 の担当**。JIS C 0617 は電気用図記号（展開図・結線図・単線結線図）で、正典 [計装工事で押さえるJIS規格](../03-keiso/jis-standards.md) が両者を明確に分けている。索引側だけ混ざっていたため是正し、JIS Z 8204 の行を追加した |
...
### 条文リンク（e-Gov 法令検索・2026-09-18 閲覧）

本節で引用した条文の所在です。e-Gov のリンクは現行条文で、2026-09-18 に e-Gov 法令 API（v1）で取得した本文と本節の引用内容が一致することを再確認しました。アンカー ID は e-Gov 法令 API の XML 階層（章・節・款）から算出しています。ただし e-Gov は読み込み直後に該当条まで自動でスクロールしないことがあります。その場合は画面左の目次または法令内検索で条番号へ移動してください。
