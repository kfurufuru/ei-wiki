...
!!! example "Samsung（海外電子機器メーカー）の事例"
    2023年3月、海外の電子機器メーカーで社内情報の流出が立て続けに3件発生した。
    ...
    **なぜ漏洩したか：** 使用したサービスが「入力データを学習に使う可能性がある」個人向けサービスだったため。

    この事例の後、多くの企業が社員への生成AI利用ガイドラインを整備・強化した。

!!! danger "社外秘を誤って入力してしまったら（初動フロー）"
    禁止情報を誤って外部AIに入力してしまった場合、隠さず速やかに動く。

    1. **直ちに入力を中止し、チャット（入力内容）を削除する**
    2. **速やかに（社内基準の時限内に）情報セキュリティ担当・上長へ一次報告する**
    3. **何を・どのサービスに入力したかを記録して提出する**

    自己判断で隠したり様子見したりしない。初動の速さが被害拡大を防ぐ。
...
!!! info "「要IT確認」「IT許可後」で確認すべきチェック項目"
    表で「要IT確認」「IT許可後」となっているツールは、以下の4点を確認してから使う。

    1. **入力データの送信先と保存場所**（社内に留まるか／社外へ出るか）
    2. **学習利用の有無**（入力がモデルの学習に使われないか）
    3. **ログの保持期間**（入力内容がどこに・いつまで残るか）
    4. **社外秘の取扱可否**（社外秘を入力してよい契約・設定か）
