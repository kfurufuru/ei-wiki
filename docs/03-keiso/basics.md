...
!!! info "NAMUR NE43：測定レンジ端と故障判定は別レベル"
    上表の 3.8mA / 20.5mA は有効測定レンジの端（アンダー／オーバーレンジ警報）。さらに外側の **≤3.6mA（下限故障）／≥21mA（上限故障）** が NE43 の故障判定レベルで、断線・短絡・センサ異常を示す。「レンジ端の警報（3.8/20.5）」と「故障判定（3.6/21）」は別段階なので、他ページで両者が併記されていても矛盾ではない。
...
!!! warning "配線抵抗の上限"
    ...
    ただし最低動作電圧は機器により異なり（10.5〜17V程度と幅がある）、電源電圧も24Vとは限らない。上記600Ωはあくまで24V/12V前提の計算例であり、実際の設計では必ず伝送器仕様書の値を使うこと。
