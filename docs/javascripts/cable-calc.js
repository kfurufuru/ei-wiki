...
(function () {
  ...
  // R は参考値（カタログ照合用）であり、電圧降下の計算には使わない（下記 R90）。
  //
  // R90_50 / R90_60: 90℃ の交流実効抵抗（Ω/km、50Hz / 60Hz）。電圧降下の計算はこちらを使う。
  // 日本電線工業会 技資第103号B 表③（50Hz）・表④（60Hz）の「R(90℃)」列の値で、
  // 2026-09-17 に原本 PDF（8〜9 ページ）で全サイズ・両周波数を直接照合した。
  // 同資料 5 章の計算例①も R(90℃)（60mm² で 0.397）で電圧降下を計算している。
  // 表皮効果・近接効果を含むため 38mm² 以上では周波数でわずかに変わり、20℃ 値に温度係数を
  // 掛けただけの値（250mm² で 0.0961）より大きい。温度換算で代用しないこと。
  // 2026-08-02〜09-17 の本ツールは R(20℃) で計算しており、電圧降下を 2 割強小さく出していた。
  //
  // X50 / X60: リアクタンス（Ω/km、50Hz / 60Hz）。日本電線工業会 技術資料
  // 技資第103号B「低圧電線・ケーブルのインピーダンス」(2024年6月) 表③（50Hz）・
  // 表④（60Hz）の規格値。同資料は無償公開で 2026-08-02 に原本 PDF を直接照合した
  // （https://www.jcma2.jp/files/gijutsu/Shiryo/103B.pdf）。
  //   cv  … 「2心及び3心 一括シース形」の列
  //   cvt … 「デュープレックス形・トリプレックス形」の列
  // X はケーブル構成（導体間隔・導体径）と周波数で決まる幾何量で、CVT は導体間隔が
  // 大きいぶん CV 3心より 2〜3割高い（14mm² 50Hz で 0.107 vs 0.0828）。旧データの
  // 「X = 0.09 固定」は全サイズ・両種別・両周波数のどれとも一致せず出所不明だった。
  // 温度では変わらないため、R(90℃) と組み合わせてそのまま使う。
  var CABLE_DB = {
    cv:  [
      { size: 2.0,  amp: [19,  23,  23],  R: 9.42,   R90_50: 12.0,   R90_60: 12.0,   X50: 0.0992, X60: 0.119  },
      { size: 3.5,  amp: [26,  33,  33],  R: 5.30,   R90_50: 6.76,   R90_60: 6.76,   X50: 0.0914, X60: 0.110  },
      { size: 5.5,  amp: [35,  44,  44],  R: 3.40,   R90_50: 4.34,   R90_60: 4.34,   X50: 0.0914, X60: 0.110  },
      { size: 8,    amp: [43,  54,  54],  R: 2.36,   R90_50: 3.01,   R90_60: 3.01,   X50: 0.0870, X60: 0.104  },
      { size: 14,   amp: [59,  76,  76],  R: 1.34,   R90_50: 1.71,   R90_60: 1.71,   X50: 0.0828, X60: 0.0994 },
      { size: 22,   amp: [77,  100, 100], R: 0.849,  R90_50: 1.08,   R90_60: 1.08,   X50: 0.0820, X60: 0.0984 },
      { size: 38,   amp: [110, 140, 140], R: 0.491,  R90_50: 0.626,  R90_60: 0.627,  X50: 0.0771, X60: 0.0925 },
      { size: 60,   amp: [150, 190, 190], R: 0.311,  R90_50: 0.397,  R90_60: 0.397,  X50: 0.0768, X60: 0.0922 },
      { size: 100,  amp: [210, 260, 260], R: 0.187,  R90_50: 0.239,  R90_60: 0.240,  X50: 0.0773, X60: 0.0928 },
    ],
    cvt: [
      { size: 14,   amp: [63,  86,  86],  R: 1.34,   R90_50: 1.71,   R90_60: 1.71,   X50: 0.107,  X60: 0.128  },
      { size: 22,   amp: [82,  110, 110], R: 0.849,  R90_50: 1.08,   R90_60: 1.08,   X50: 0.103,  X60: 0.123  },
      { size: 38,   amp: [110, 155, 155], R: 0.491,  R90_50: 0.626,  R90_60: 0.626,  X50: 0.0955, X60: 0.115  },
      { size: 60,   amp: [150, 210, 210], R: 0.311,  R90_50: 0.397,  R90_60: 0.397,  X50: 0.0913, X60: 0.110  },
      { size: 100,  amp: [215, 290, 290], R: 0.187,  R90_50: 0.239,  R90_60: 0.240,  X50: 0.0881, X60: 0.106  },
      { size: 150,  amp: [275, 380, 380], R: 0.124,  R90_50: 0.159,  R90_60: 0.160,  X50: 0.0846, X60: 0.102  },
      { size: 200,  amp: [340, 465, 465], R: 0.0933, R90_50: 0.121,  R90_60: 0.121,  X50: 0.0859, X60: 0.103  },
      { size: 250,  amp: [395, 535, 535], R: 0.0754, R90_50: 0.0981, R90_60: 0.0989, X50: 0.0836, X60: 0.100  },
    ]
  };
  ...
  window.calcCable = function () {
    ...
    // 周波数でリアクタンスが 2 割変わる（14mm² CV で 0.0828 → 0.0994）。
    // セレクタが無い旧ページで読み込まれた場合は保守側の 60Hz を既定にする。
    var is50       = (g('cc_freq') && g('cc_freq').value === '50');
    var xKey       = is50 ? 'X50' : 'X60';
    // 電圧降下は 90℃ の交流実効抵抗で計算する（技資第103号B の設計基準。20℃ 値は使わない）
    var rKey       = is50 ? 'R90_50' : 'R90_60';
    ...
    for (var i = 0; i < CABLE_DATA.length; i++) {
      ...
      var vd        = factor * I * (d[rKey] * pf + d[xKey] * sinθ) * L;
    ...
    warn.innerHTML =
      ...
      '電圧降下は <b>90℃ の交流実効抵抗 R(90℃)</b>（技資第103号B）で計算しています。' +
