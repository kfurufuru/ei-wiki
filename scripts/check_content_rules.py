...
FORBIDDEN = [
    ...
    # N34-N38 は並列 PR #46（N34-N36）・#50（N37-N38）が使用中のため欠番とし、N39 から採番。
    # N39-N40: 2026-07-30 に電技解釈（令和7年11月版）原典PDFで逐語照合して確定。
    # N39: 回転機（モーター）の直流耐圧代替は「16-2表の試験電圧の1.6倍」（解釈第16条第2項第二号）。
    # 「交流の2倍」は電線にケーブルを使用する電路（第15条第二号）の倍率で、回転機に適用すると
    # 過大印加になる。正しい是正文は同一行に「1.6」を併記するため、1.6 を含む行は対象外（負対照）。
    ("N39", (lambda line: bool(re.search(r"回転機|モーター|電動機", line))
...
>>>>>>> 4f06dc8 (add(lint): 今回是正した耐圧の誤り2件を N39-N40 に登録（N34-N38 は並列PRが使用中のため欠番）)
]
...
def self_test():
    ...
    # 3f) N39/N40 負対照: 是正済みの正しい説明文（1.6倍併記・第16条併記）を誤検出しないこと。
    ...
    for s in [
        ...
        if hits & {"N39", "N40"}:
            failures.append(f"N39/N40 負対照: 正しい説明文 {s!r} を誤検出（{hits & {'N39', 'N40'}}）")
