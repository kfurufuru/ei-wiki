...
FORBIDDEN = [
    ...
    # N37-N38: 2026-07-30 の 04-sekkei 計算系ページ再検証で確定。
    # N34-N36 は open PR #46（09-hoantokei）が使用するため空けてある。
    # N37: 電動機始動時の許容電圧降下 10〜15% に該当する JEM 規格番号は一次確認できなかった。
    # 根拠なき規格番号（R5）を書かせない。「JEM の裏付けは確認できない」と正しく
    # 否定する是正文には当たらないよう、否定形の語を _both_unless で除外する。
    ("N37", _both_unless(r"始動", r"JEM",
    ...
    # N38: 需要率の定義は「最大需要電力 ÷ 設備容量」（電験・配電設計の標準定義）。
    # 分子を「実際の使用電力」と書くと瞬時使用電力と紛れ、最大需要電力の算定を誤る。
    ("N38", re.compile(r"需要率.{0,10}実際の使用電力"),
