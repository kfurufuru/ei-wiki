...
- **SIL の定義帯（PFDavg・PFH）**: JIS C 0508-1:2012 表2（低頻度作動要求モード: SIL 1＝10⁻²以上10⁻¹未満 〜 SIL 4＝10⁻⁵以上10⁻⁴未満）・表3（高頻度作動要求または連続モード: SIL 1＝10⁻⁶以上10⁻⁵未満 〜 SIL 4＝10⁻⁹以上10⁻⁸未満、単位 1/h）。JIS C 0508-1 は IEC 61508-1:2010 の対応規格。規格全文の転載サイトで **2026-07-30 に逐語照合**（JSA/IEC 発行の原本そのものは未照合）。
- **PFDavg の近似式**: JIS C 0508-6:2019 附属書B（1oo1 の式は PFDG＝(λDD＋λDU)×tCE、tCE＝T1/2＋MRT）を転載サイトで確認。本文の「λDU×TI/2」「(λDU×TI)²/3＋β×λDU×TI/2」は**修復時間・診断修復を無視した簡易形**であり、規格の式そのものではない（桁の把握用。本文の警告どおり、この式だけで SIL を主張しないこと）。
