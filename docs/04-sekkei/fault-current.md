...
電源から短絡点まで、各要素の %Z を同じ基準容量で直列に足し合わせる。合計 %Z が小さいほど短絡電流は大きくなる。

```mermaid
flowchart LR
    G[電源<br/>系統] -->|%Z系統| T[変圧器<br/>一次]
    T -->|%Z変圧器| C[変圧器<br/>二次]
    C -->|%Zケーブル| F((短絡点))
    F --> R["%Z合計 = %Z系統 + %Z変圧器 + %Zケーブル<br/>Is3 = Ib × 100 / %Z合計"]
```

*電源→系統→変圧器→ケーブルの %Z を直列合成し、短絡点の %Z合計 から Is3 を求める。*
