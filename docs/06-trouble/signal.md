...
> 参照: 断線・異常出力の閾値（3.6mA以下 / 21mA以上）は NAMUR NE43 準拠機器の代表値。機器設定により異なる場合がある。
...
SIS のバイパスは安全機能を一時的に無効化するため、開始から解除までの承認・記録・役割を明確にする。

| 項目 | 運用 |
|------|------|
| 承認 | バイパスの開始は安全担当の承認を得てから実施する |
| 許容時間 | バイパスの許容時間の上限を作業前に設定し、超過見込みなら再承認を得る |
| 監視 | バイパス中は指名した監視員が対象プロセスを常時監視する |
| 記録 | 開始・解除の時刻と承認者を SIS 管理台帳（記録先）に記録する |
| 解除確認 | 解除後にバイパスが確実に解除されたことを確認・記録する |
