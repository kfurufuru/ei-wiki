...
雷保護は **IEC 62305 シリーズ**で体系化され、**外部雷保護システム（LPS）** と **内部雷保護** に分かれます。計装担当が主に関与するのは内部雷保護です。国内には JIS A 4201「建築物等の雷保護」がありますが、**同 JIS と IEC 62305 の対応関係・版は本Wikiでは未照合**のため、両者を同一視せず併記にとどめます（下記「根拠」参照）。
...
!!! tip "段間の協調条件"
    上流と下流の SPD の間には協調のための電路長が必要です。
    実務では **SPD 間に 10 m 程度の電路長**（デカップリングインダクタを入れる場合は 5 m 程度）を確保する、という目安が
    SPD メーカーの選定資料で広く示されています。近接して直付けすると両者が同時動作して分担が崩れるためです。
    ただし**この 10 m / 5 m を規格の要求として引用できる原本を本Wikiは照合していません**（下記「根拠」参照）。
    協調は電路長だけで決まるものではないため、**製造者が「協調済みセット」として提供する組み合わせを使う**か、
    製造者の協調表で確認するのが確実です。
...
- **IEC 62305 シリーズ** — 雷保護システム全体・LPZ・等電位ボンディング
- **IEC 61643-11** — 低圧電源用 SPD の分類・試験（Type 1/2/3）
- **IEC 61643-21** — 通信・信号回線用 SPD
- **JIS A 4201「建築物等の雷保護」** — 国内の建築物雷保護
- **JIS C 5381 シリーズ** — 国内の SPD 規格

!!! warning "IEC と JIS を同一視しない"
    上の IEC 規格と JIS 規格は**対応関係・版が一致しているとは限りません**。
    本Wikiはいずれの原本も照合していないため、「IEC 62305 ＝ JIS A 4201」のような等号は置かず併記にとどめます。
    設計適用時は JIS 原文・IEC 原文・製品の適合宣言書で、**規格番号と版を対で**確認してください。
...
- **高圧・特別高圧の電路に施設する避雷器にはA種接地工事**: 電技解釈 **第37条第3項**（原典 PDF「電気設備の技術基準の解釈」令和7年11月版で 2026-08-02 に逐語照合）。避雷器の施設箇所は同条第1項（発電所・変電所等の引込口／引出口、配電用変圧器の高圧側・特別高圧側、高圧架空電線路から受電する **受電電力 500 kW 以上**の需要場所の引込口、特別高圧架空電線路から受電する需要場所の引込口）
- **避雷針（受雷部）は避雷器とは別系統**: 電技解釈 **第17条第1項第四号**が「接地線は、避雷針用地線を施設してある支持物に施設しないこと」と定めており、両者は分離して扱われます。接地種別・抵抗値の正典は [接地抵抗測定手順](../guidelines/grounding-inspection.md) です

### 原本未照合（honest-hold）

以下はいずれも**有償規格で原本を照合していません**。本ページでは節番号・表番号・版年を挙げず、内容も断定形を避けています。

- **IEC 62305 シリーズ**（LPZ の区分 0A/0B/1/2、外部LPS と内部雷保護の区分、等電位ボンディングの要求）
- **IEC 61643-11 / JIS C 5381-11**（Type 1/2/3 の分類と試験波形 10/350 µs・8/20 µs・複合波、In の「15回」という繰り返し回数、Uc・In・Imax・Up の定義）
- **IEC 61643-21 / JIS C 5381-21**（信号・通信線用 SPD、本文に記した適用範囲「公称電圧 1000 V a.c. / 1500 V d.c. 以下」）
- **JIS A 4201「建築物等の雷保護」**（IEC 62305 との対応関係・版も含めて未照合）
- **段間協調の 10 m / 5 m**: SPD メーカーの選定資料で広く示される実務目安として記載しています。**規格の要求として引用できる原本は照合していません**

### 是正の記録（2026-08-02）

| 箇所 | 修正前 | 修正後 |
|---|---|---|
| 規格の対応づけ | 「IEC 62305（**JIS A 4201**「建築物等の雷保護」）」と等号で併記 | 対応関係が未照合のため**等号を外して併記**に変更 |
| 段間協調の距離 | 「**IEC 62305-4 は** SPD 間に最低 10 m の電路長を推奨」と規格に帰属 | 原本未照合のため**メーカー選定資料の実務目安**として記載し、協調表での確認を促す形に変更 |

### 限界の明示

- 本ページは**内部雷保護（SPD と等電位ボンディング）**に絞ります。外部LPS の設計（保護レベル LPL、回転球体法、受雷部・引下げ導線の配置）は扱いません
- SPD の**具体的な選定値**（Iimp・In・Up の数値）は製品ごとに異なるため、本文の「代表値の目安」は目安であって選定根拠にはなりません。必ず製品カタログと被保護機器の耐サージ電圧 Uw を突き合わせてください
- 接地抵抗値・接地種別は本ページで再定義せず、[接地抵抗測定手順](../guidelines/grounding-inspection.md)・[接地（低圧）](../02-teiatsu/grounding-lv.md) を正典とします
