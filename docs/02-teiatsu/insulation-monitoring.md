...
!!! info "法令上の位置づけ — 警報値は法定値ではない"
    低圧電路の絶縁性能は電気設備技術基準で定める水準（電技省令第58条の絶縁抵抗下限。数値と適用区分の正典は [絶縁管理](../05-hozen/insulation-management.md)）を満たす必要があり、絶縁監視はその維持・確認を運転中に支援する手段と位置づけられます。**監視装置の警報値（mA）は法令が定める値ではなく、現場で設定する管理値**です。第58条は「開閉器・過電流遮断器で区切ることのできる電路ごと」の停電測定を前提とした絶縁抵抗の下限であり、常時監視の漏洩電流としきい値の次元が異なる点を混同しないでください（停電測定が困難な場合の漏えい電流による代替判定は電技解釈第14条側の規定。逐語照合済みの引用は絶縁管理ページ参照）。常時監視装置の設置自体が一律に義務づけられているわけではないため、自社の保安規程・設備の重要度に応じて導入を判断します。
...
- 絶縁監視装置（Io 方式／Ior 方式）の測定原理・方式選定・警報設定・導入運用ステップ。

### 一次計算で検算済み

- 漏洩電流の分解 **Io = √(Ioc² + Ior²)**：Ioc（容量分）は対地電圧より位相が 90 度**進み**、Ior（抵抗分）は同相。直交する 2 成分のベクトル和として三平方の定理どおり（2026-08-06 検算、進み／遅れの向きの取り違えなし）。
- 「Ioc が大きいと Ior の増加が Io に埋もれる」：Io = √(Ioc² + Ior²) で Ioc ≫ Ior のとき ∂Io/∂Ior = Ior/Io ≪ 1 となり、Ior が増えても Io がほとんど動かないことを微分で確認。見逃し・誤報の両方向の説明も式と整合。

### 実務慣行（規格・法令の裏付けなし）

- 2 段警報の構成、「平常時 Ior の 2〜3 倍程度を注意とする」等の設定目安は実務慣行・メーカー推奨の領域であり、法令・規格が定める値ではない（本文でも明示済み）。
- 重要回路の選定基準・ベースライン測定期間の取り方も実務慣行。

### 是正の記録（2026-08-06）

- 「法令上の位置づけ」の info を具体化：**監視装置の警報値（mA）は法定値ではない**こと、電技省令第58条（低圧電路の絶縁抵抗下限・停電測定前提）と常時監視の漏洩電流監視は**次元が異なる**ことを明示し、数値そのものは正典（絶縁管理ページ）への参照とした。
- 上記以外（Io/Ioc/Ior の定義と位相関係・Ior 方式の優位性の説明・ZCT 取付位置と監視範囲の注意）は照合したがずれ無し。

### 原本未照合（honest-hold）

- 絶縁監視装置による遠隔常時監視を月次点検の頻度緩和に使う制度運用（保安規程・告示レベルの扱い）は本ページでは扱わず、記述もしていない。導入で点検頻度を変える場合は所管への確認と保安規程の改定を経ること。

### 正典参照

- 低圧電路の絶縁抵抗下限（電技省令第58条）の数値・適用区分と実務管理値、漏えい電流による代替判定（電技解釈第14条）の逐語照合 → [絶縁管理](../05-hozen/insulation-management.md)
- 停電して測るメガー測定・印加電圧・PI 値 → [耐圧試験・絶縁診断](../01-koatsu/insulation-test.md)
- B 種接地・接地系統の前提 → [接地（低圧）](grounding-lv.md)

照合日: 2026-08-06（Io = √(Ioc² + Ior²) と位相関係を検算し、警報値と省令第58条の射程の違いを明示。数値境界は絶縁管理ページへの参照に統一）。
...
- [絶縁管理](../05-hozen/insulation-management.md) — 絶縁抵抗の判定境界（電技省令第58条・実務管理値）の正典
