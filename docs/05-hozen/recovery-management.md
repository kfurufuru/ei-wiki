...
行は製造部門の A ランク設備リストをそのまま使い、1 行 ＝ 1 故障モードにします。故障モードの割り方（打ち手が変わる単位で割る）と機器種別ごとの代表モードは [故障モード棚卸し](failure-mode-inventory.md) が正典です。設備単位の復旧時間は各モードの最大値です。まず表計算ソフトで埋まることを確認し、設備管理システム（CMMS）への登録は充足率が上がってから判断します。
...
| 故障モードの割り方・機器種別ごとの代表モード・設定情報の復元性 | [故障モード棚卸し](failure-mode-inventory.md) |
...
- [故障モード棚卸し](failure-mode-inventory.md) — 本表の行（1 行 ＝ 1 故障モード）をどの単位で起こすか
