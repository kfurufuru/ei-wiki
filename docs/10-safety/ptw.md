...
sequenceDiagram
    ...
    participant S as 監督者
    participant M as 設備管理者（電気担当）
    W->>S: ①申請（作業内容・停電範囲記入）
    S->>M: ②審査（安全措置の確認）
    M->>S: ③承認（停電措置実施後に承認）
    S->>W: ④許可書交付
    Note over W: ⑤作業実施（許可書携帯）
    W->>S: ⑥完了報告
    S->>M: ⑦完了確認
    Note over M: 復電・許可書返却・保管
```
...
- 検電器の種類：低圧回路には低圧用、高圧回路には高圧用の検電器を使う。**低圧用検電器で高圧を確認してはならない（無反応でも活線の恐れ）**。検電器の定格電圧が対象電圧を含むことを毎回確認
...
- 緊急時フロー（口頭承認→事後書類整備）は社内手順書に従う
- 口頭承認は当直責任者または設備管理者が行い、**承認者名・時刻・停電範囲を口頭承認の時点で記録**する
- 作業後は速やかに（社内規定の期限内に）正式な作業許可書を作成し、設備管理者へ提出・保管する（具体的な期限は社内手順書に従う）
