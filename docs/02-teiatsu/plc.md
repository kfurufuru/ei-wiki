...
| MODBUS RTU | シリアル（RS-485）、代表 9.6k〜115.2kbps（設定可変） | 計装機器との簡易接続 |
...
- 動作確認 NG 時のロールバック：直ちに変更前バックアップへ復元し、当直・安全担当へ速やかに報告する。原因究明が完了するまで再変更は禁止（対応順序・報告先・時限は社内 MOC 規定に従う）
